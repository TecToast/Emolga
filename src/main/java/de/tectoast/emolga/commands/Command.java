...
public abstract class Command {
     ...
     */
    public static final HashMap<String, Translation> translationsCacheGerman = new HashMap<>();
    /**
     ...
     */
    public static final HashMap<String, Translation> translationsCacheEnglish = new HashMap<>();
    /**
     * Order of the cached items, used to delete the oldest after enough caching
     */
    ...
    public static final HashMap<Long, TrainerData> trainerDataButtons = new HashMap<>();
    public static final HashMap<Long, MonData> monDataButtons = new HashMap<>();
    public static final HashMap<Long, Nominate> nominateButtons = new HashMap<>();
        ...
        socket.on("herokuemolga", args -> {
            ...
            switch (id) {
                case "cookievalidation" -> {
                    System.out.println("msg = sid = " + msg);
    ...
    public static int indexPick(ArrayList<String> picks, String mon) {
    ...
    public static void sortNDSSingle(String sid, JSONObject league, int num) {
        ...
        List<List<Object>> points = Google.get(sid, "Tabelle RR!D" + start + ":K" + end, false, false);
        List<List<Object>> orig = new ArrayList<>(points);
        JSONObject docnames = getEmolgaJSON().getJSONObject("docnames");
        points.sort((o1, o2) -> {
            int c = compareColumns(o1, o2, 1, 7, 5);
    ...
    private static void registerCommands() {
        ...
        //new DataCommand();new MovesCommand();new CanlearnCommand();new SmogonCommand();new TierCommand();new AbilityCommand();new AttackCommand();new CombinationCommand();new GerCommand();new EnglCommand();new SpeedCommand();new AllLearnCommand();new BerryCommand();new NicknameCommand();new AslsortCommand();new CountuntilCommand();new DeleteuntilCommand();new LigaCommand();new ListallmembersCommand();new ListmembersCommand();new CreatedraftCommand();new DeldraftCommand();new DraftsetupCommand();new PickCommand();new SetupfromfileCommand();new SpielplanCommand();new StopdraftCommand();new UpdatebattleorderCommand();new UpdatedatafromfileCommand();new UpdateorderfromfileCommand();new UpdatepicksCommand();new DexquizCommand();new SolutionCommand();new TipCommand();new ByeCommand();new ChillCommand();new QueueClearCommand();new DcCommand();new DeepCommand();new MusicCommand();new NpCommand();new PlayCommand();new QlCommand();new QueueCommand();new SkipCommand();new ReplayCommand();new TabelleCommand();new AddCommand();new CheckCommand();new AddMonCommand();new DelMonCommand();new GcreateCommand();new WarnCommand();new WarnsCommand();new MuteCommand();new UnmuteCommand();new BanCommand();new HuntCommand();new GiveMeAdminPermissionsCommand();new GetIdsCommand();new KickCommand();new TempMuteCommand();new TempBanCommand();new SetBirthdayCommand();new UpcomingBirthdaysCommand();new ClearCommand();new ResultCommand();new RoundCommand();new MuffinCommand();new ShinyCommand();new CheckMonsCommand();new EmoteStealCommand();new RandomPickCommand();new CalcCommand();new LeaderboardCommand();new InviteCommand();new ResetCooldownCommand();new SpoilerTagsCommand();new AnalyseCommand();new SearchReplaysCommand();new InviteUrlCommand();new FloHelpCommand();new EmolgaDiktaturCommand();new UserInfoCommand();new ServerInfoCommand();new SkipPickCommand();new PokeFansExportCommand();new RevolutionCommand();new ResetRevolutionCommand();new NatureCommand();new InvertColorsCommand();new ResistanceCommand();new GoinCommand();
    }
    ...
    public static void awaitNextDay() {
        ...
        System.out.println("SELECT REQUEST: " + "SELECT * FROM birthdays WHERE month = " + (c.get(Calendar.MONTH) + 1) + " AND day = " + c.get(Calendar.DAY_OF_MONTH));
        new Timer().schedule(new TimerTask() {
            @Override
            public void run() {
    ...
    public static void analyseReplay(String url, TextChannel customReplayChannel, TextChannel resultchannel, Message m, GuildCommandEvent e) {
        Player[] game;
        ...
        try {
            ...
            resultchannel.sendMessage("Beim Auswerten des Replays ist (vermutlich wegen eines Zoruas/Zoroarks) ein Fehler aufgetreten! Bitte trage das Ergebnis selbst ein!").queue();
            if (e != null)
                e.errorInteraction();
            ex.printStackTrace();
            return;
        ...
        //System.out.println(g.getName() + " -> " + (m.isFromType(ChannelType.PRIVATE) ? "PRIVATE " + m.getAuthor().getId() : m.getTextChannel().getAsMention()));
        for (Pokemon p : game[0].getMons()) {
        ...
        for (Pokemon p : game[0].getMons()) {
            System.out.println("p.getPokemon() = " + p.getPokemon());
            String monName = getMonName(p.getPokemon(), gid);
            System.out.println("monName = " + monName);
    ...
    public static Translation getDraftGerName(String s) {
        ...
        if (t.isSuccess()) {
            ...
            System.out.println("getDraftGerName ret = " + tr);
            return tr;
        }
    ...
    public static String getSDName(String str, String mod) {
        System.out.println("getSDName s = " + str);
        Optional<String> op = sdex.keySet().stream().filter(str::equalsIgnoreCase).findFirst();
        String gitname;
        if (op.isPresent()) {
            String ex = op.get();
