...
object DocEntries {
    ...
    val NDS = DocEntry.create {
        ...
        tableMapper = { nds: JSONObject ->
            nds.getStringList("table")
                .map { s: String? -> Command.reverseGet(nds.getJSONObject("teamnames"), (s)!!)!!.toLong() }
        }
        killProcessor = BasicStatProcessor { plindex: Int, monindex: Int, gameday: Int ->
            StatLocation(
                "Data",
        ...
        resultCreator = BasicResultCreator { b: RequestBuilder, gdi: Int, index: Int, _: Int, _: Int, url: String? ->
            b.addSingle(
                "Spielplan RR!${getAsXCoord(gdi * 9 + 5)}${index * 10 + 4}",
                "=HYPERLINK(\"$url\"; \"Link\")"
            )
        }
        setStatIfEmpty = false
